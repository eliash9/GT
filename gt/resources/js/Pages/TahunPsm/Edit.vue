...
const form = useForm({
    judul: props.tahunPsm.judul || '',
    tanggal_mulai_masehi: props.tahunPsm.tanggal_mulai_masehi || '',
    tanggal_selesai_masehi: props.tahunPsm.tanggal_selesai_masehi || '',
    tanggal_mulai_hijriah: props.tahunPsm.tanggal_mulai_hijriah || '',
    tanggal_selesai_hijriah: props.tahunPsm.tanggal_selesai_hijriah || '',
    aktif: props.tahunPsm.aktif || false,
});
...
const getHijriDate = (dateString: string) => {
    ...
    return `${m.iDate()} ${hijriMonths[m.iMonth()]} ${m.iYear()}`;
};

watch(() => form.tanggal_mulai_masehi, (val) => {
    if (val) form.tanggal_mulai_hijriah = getHijriDate(val);
    else form.tanggal_mulai_hijriah = '';
});

watch(() => form.tanggal_selesai_masehi, (val) => {
    if (val) form.tanggal_selesai_hijriah = getHijriDate(val);
    else form.tanggal_selesai_hijriah = '';
});

const submit = () => {
    form.put(route('tahun-psm.update', props.tahunPsm.id));
};
</script>

<template>
    <Head title="Edit Tahun PSM" />
    <AuthenticatedLayout>
        <div class="max-w-2xl space-y-5 mx-auto">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Tahun PSM</h1>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Pembaruan master data periode.</p>
                </div>
            </div>
            ...
            <form @submit.prevent="submit" class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input v-model="form.tanggal_mulai_hijriah" type="text"
                            class="input block w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input v-model="form.tanggal_selesai_hijriah" type="text"
                            class="input block w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    </div>
                </div>
                ...
                <div class="pt-2">
                    <label class="flex items-center gap-3 cursor-pointer">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-0.5">Tahun PSM aktif akan diset sebagai default otomatis.</p>
                        </div>
                    </label>
                </div>
                ...
                <div class="pt-4 border-t border-gray-100 dark:border-gray-700 flex gap-3">
                    <button type="submit" :disabled="form.processing"
                        ...
                        {{ form.processing ? 'Menyimpan...' : 'Perbarui Tahun PSM' }}
                    </button>
