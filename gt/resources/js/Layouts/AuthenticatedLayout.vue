...
const navItems = computed((): NavItem[] => {
    ...
    if (can('view lembagas') || can('view wilayahs') || can('view pjgts') || can('view skills')) {
        items.push({
            ...
            children: [
                ...
                ...(can('view lembagas') ? [{ label: 'Peta Sebaran', route: 'lembagas.sebaran' }] : []),
    ...
    if (can('manage attendance sessions')) {
        items.push({
            label: 'Absensi & Laporan', icon: 'calendar',
            children: [
                { label: 'Manajemen Sesi', route: 'attendance-sessions.index' },
                { label: 'Rekap & Laporan', route: 'attendance-reports.index' },
            ]
        });
    }

    items.push({
        label: 'Analisa Laporan', icon: 'clipboard',
